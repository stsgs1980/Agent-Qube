...
const dotColors: Record<ActivityKind, string> = {
...
function DefaultIcon({ kind }: { kind: ActivityKind }) {
...
/**
 * ActivityTimeline -- vertical timeline of recent activity entries.
 *
 * Displays a chronological list of events with color-coded indicators,
 * relative timestamps, and optional detail text. Extracted from
 * Component-Browser's ActivityTimeline pattern.
 *
 * @example
 * ```tsx
 * <ActivityTimeline
 ...
 *     { id: '2', description: 'Build failed', kind: 'error', timestamp: '2024-01-15T10:30:00Z' },
 *   ]}
 * />
 * ```
 */
export const ActivityTimeline = forwardRef<HTMLDivElement, ActivityTimelineProps>(
  ({ entries, heading, maxItems, emptyMessage = 'No activity yet', renderIcon, className, ...props }, ref) => {
    const visible = useMemo(
      () => maxItems ? entries.slice(0, maxItems) : entries,
      [entries, maxItems]
    )
              ...
              return (
                <div key={entry.id} className="relative flex gap-3 pl-1">
                  ...
                  <span className={cn(
                    ...
                    {renderIcon ? renderIcon(kind) : <DefaultIcon kind={kind} />}
                  </span>
