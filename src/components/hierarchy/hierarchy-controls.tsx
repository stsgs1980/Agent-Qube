...
import {
  Search, LayoutGrid, Circle, Grid3X3, Layers, ArrowUpDown,
  ZoomIn, ZoomOut, Maximize2, Crosshair,
} from 'lucide-react'
import { EDGE_CONFIG, type EdgeType, type ViewMode } from './types'

// ─── Toolbar with view mode, layers, search, edge filters, zoom ─────────────────
...
export function HierarchyControls({
  ...
  fitMode, selectedAgentId, handleFitView, handleFocus, reactFlowInstanceRef,
}: {
  viewMode: ViewMode; setViewMode: (m: ViewMode) => void
  showLayers: boolean; setShowLayers: (s: boolean) => void
  searchQuery: string; setSearchQuery: (q: string) => void
  visibleEdgeTypes: Set<EdgeType>; toggleEdgeType: (t: EdgeType) => void
  fitMode: boolean; selectedAgentId: string | null
  handleFitView: () => void; handleFocus: () => void
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <button onClick={() => setShowLayers(!showLayers)} style={{
          ...
          color: showLayers ? '#06B6D4' : '#555', cursor: 'pointer', textTransform: 'uppercase',
          letterSpacing: 0.5, display: 'flex', alignItems: 'center', gap: 3,
        }} title="Toggle layer labels"><Layers size={9} />Layers</button>
        <button onClick={() => { const m = viewMode; setViewMode('grid'); setTimeout(() => setViewMode(m), 50) }} style={{
          padding: '3px 8px', borderRadius: 4, fontSize: 9, fontWeight: 600,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {(Object.entries(EDGE_CONFIG) as [EdgeType, typeof EDGE_CONFIG[EdgeType]][]).map(([type, cfg]) => (
            <button key={type} onClick={() => toggleEdgeType(type)} style={{
              padding: '2px 6px', borderRadius: 3, fontSize: 8, fontWeight: 600, cursor: 'pointer',
              color: cfg.color, opacity: visibleEdgeTypes.has(type) ? 1 : 0.35,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <button onClick={handleFitView} style={{
          ...
          color: fitMode ? '#06B6D4' : '#888', cursor: 'pointer', display: 'flex',
          alignItems: 'center', gap: 3, textTransform: 'uppercase', transition: 'all 0.15s',
        }} title={fitMode ? 'Fit mode ON' : 'Fit mode OFF'}><Maximize2 size={9} />Fit<span style={{ fontSize: 7, opacity: 0.7, marginLeft: 1 }}>{fitMode ? 'ON' : 'OFF'}</span></button>
        <button onClick={handleFocus} style={{
          ...
          color: selectedAgentId ? '#06B6D4' : '#555', cursor: selectedAgentId ? 'pointer' : 'not-allowed',
          display: 'flex', alignItems: 'center', gap: 3, fontSize: 9, fontWeight: 600, textTransform: 'uppercase',
        }} title="Focus on selected node"><Crosshair size={9} />Focus</button>
      </div>
    </div>
  )
}
