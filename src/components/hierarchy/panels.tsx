...
// ─── Detail Panel (thin orchestrator) ──────────────────────────────────────
...
export function DetailPanel({
  ...
  const mutations = useAgentMutations({
    onAgentUpdated: (a) => onAgentUpdated?.(a as AgentData),
    onAgentDeleted,
  ...
  if (editMode) {
    return (
      <DetailPanelEdit
        ...
        onToggle={onToggle}
        onCancel={() => { setEditMode(false); mutations.setShowDeleteConfirm(false) }}
  ...
  return (
    <div style={{ background: '#0A0A0A', borderLeft: '1px solid rgba(51,51,51,0.25)', flexDirection: 'column' }} className="terrain-scroll hidden-mobile lg-flex lg-w-280 lg-flex-shrink-0 lg-overflow-y-auto overflow-y-auto">
      <AgentDetailHeader
        agent={agent}
        onToggle={onToggle}
        onEdit={() => { form.populateFromAgent(agent); setEditMode(true) }}
        onClose={onClose}
      />
