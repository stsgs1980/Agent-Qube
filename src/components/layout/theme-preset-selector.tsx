...
import { useLayoutTheme, getByMode, getAllPresets } from '@/lib/layout/theme'
...
export function ThemePresetSelector() {
  const { preset, setPreset, tokens, mode } = useLayoutTheme()
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <button onClick={() => setOpen(!open)} aria-label='Select theme' aria-haspopup='listbox' aria-expanded={open} style={{
        ...
        <div style={{ width: 10, height: 10, borderRadius: 2, background: current.accent, border: `1px solid ${tokens.borderBright}` }} />
        {current.label}
      ...
      {open && (
              ...
              return (
                <button key={p.id} onClick={() => { setPreset(p.id); setOpen(false) }} role="option" aria-selected={active} aria-label={p.label} style={{
                  ...
                  <div style={{
                    ...
                    background: p.bg,
                    border: `2px solid ${active ? p.accent : tokens.borderDefault}`,
                    position: 'relative', overflow: 'hidden', flexShrink: 0,
                  }}>
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '40%', background: p.accent, opacity: 0.3 }} />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: fontSize.base, fontWeight: active ? fontWeight.bold : fontWeight.medium, color: active ? p.accent : tokens.textPrimary, fontFamily: tokens.fontFamilyBody }}>{p.label}</div>
                    <div style={{ fontSize: fontSize.xs, color: tokens.textDim, fontFamily: tokens.fontFamilyMono, marginTop: 1 }}>{p.description}</div>
                  </div>
                  {active && (
          ...
          <div style={{
            ...
            <span>Toggle:</span>
            <kbd style={{ fontSize: fontSize.xs, background: tokens.bgSurface, padding: '1px 4px', borderRadius: 2, border: `1px solid ${tokens.borderDefault}` }}></kbd>
