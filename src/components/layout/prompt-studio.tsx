      ...
      <div style={{ width: '100%', maxWidth: 600, marginBottom: spacing.xl }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: spacing.md, padding: `${spacing.md}px ${spacing.lg}px` }}>
            ...
            <button onClick={handleSubmit} disabled={!prompt.trim() || ai.aiLoading} aria-label={ai.aiLoading ? 'Обработка...' : ai.aiMode ? 'AI генерация' : 'Генерация'} style={{
              ...
              transition: 'all 0.2s', minHeight: 44 /* WCAG 2.5.5 */,
            }}>
          ...
          <div style={{ padding: `${spacing.sm}px ${spacing.lg}px`, borderTop: `1px solid ${tokens.borderSubtle}`, display: 'flex', alignItems: 'center', gap: spacing.md }}>
            <button onClick={toggleAiMode} aria-label={`AI Mode ${ai.aiMode ? 'включен' : 'выключен'}`} aria-pressed={ai.aiMode} style={{
              ...
              cursor: 'pointer', transition: 'all 0.15s', minHeight: 44 /* WCAG 2.5.5 */,
            }}>
            ...
            {ai.aiConfidence !== null && ai.submitted && (
              <span style={{ marginLeft: 'auto', fontSize: fontSize.sm, fontWeight: fontWeight.bold, color: tokens.accentAI, fontFamily: tokens.fontFamilyMono }}>confidence: {ai.aiConfidence}%</span>
            )}
          </div>
        </div>
      </div>
      ...
      {!ai.submitted && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.sm, justifyContent: 'center', maxWidth: 600, marginBottom: spacing['2xl'] }}>
          {PROMPT_EXAMPLES.slice(0, 5).map((ex, i) => (
            <button key={i} onClick={() => { setPrompt(ex); handleSubmit() }} aria-label={ex} style={{
        ...
        <div style={{ width: '100%', maxWidth: 896 }}>
          ...
          {top3.length > 1 && (
            <div style={{ marginTop: spacing.xl }}>
              ...
              <div key={r.structure} role="button" tabIndex={0} aria-label={`${r.recipe.name}, score ${r.score}`} style={{ border: `1px solid ${tokens.borderSubtle}`, background: tokens.bgBase, borderRadius: radius['2xl'], overflow: 'hidden', minHeight: 44 /* WCAG 2.5.5 */ }}>
