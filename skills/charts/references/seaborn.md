...
|---------|-----------|--------------|
| Histogram/KDE distribution | [OK] `histplot` / `kdeplot` | Works, but more code |
| Box plot/Violin plot | [OK] One-liner | Works, but rougher style |
| Regression scatter + confidence interval | [OK] `regplot` auto-calculates | Manual fitting + plotting |
| Correlation heatmap | [OK] `heatmap` + mask | Manual `imshow` tedious |
| Pairwise relationship matrix | [OK] `pairplot` unique | No equivalent |
| Facet grid | [OK] `FacetGrid` | `plt.subplots` manual loop |
| Regular bar chart | Less flexible than matplotlib | [OK] More control |
| Line trend chart | Less than matplotlib | [OK] More control |
| Custom annotations/arrows | Not suitable | [OK] `ax.annotate` |
