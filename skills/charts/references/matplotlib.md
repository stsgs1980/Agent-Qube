...
def add_value_labels(ax, bars, values, highlight_idx=None, fmt='{:,.0f}',
    ...
    [!] Automatically extends Y-axis upper limit to ensure labels don't overflow chart area."""
    max_val = max(values)
    ...
    # [!] Critical: extend Y-axis upper limit to leave enough space for labels (at least 15%)
...
def insight_bar(labels, values, highlight_idx, title,
    ...
    # [!] add_value_labels already sets ylim automatically, no need to repeat here
    clean_axis(ax)
    save(fig, save_path)
```
...
**Scenario**: Combine multiple subplots into a dashboard. [!] Max 4 subplots per canvas, split if exceeded.
...
```python
# [X] Wrong: 8 subplots crammed into one canvas
fig, axes = plt.subplots(2, 4, figsize=(20, 10))  # Everything becomes unreadable

# [OK] Correct: split into 2 figures, 4 subplots each
