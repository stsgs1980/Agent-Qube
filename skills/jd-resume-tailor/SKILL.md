...
- [X] 编造经历（"加上一段你没做过的项目"——绝对禁止，哪怕用户要求）
- [X] 把 JD 的整段话直接粘进简历（很容易被 HR 一眼识破，且 ATS 反作弊会标记）
- [X] 关键词堆砌（在末尾塞一长串技能词凑命中率，HR 一眼能看出）
- [X] 把"参与"改成"主导"但没有问用户实际角色 → 必须先核实
- [X] 不给用户看 gap，自己默默改 → 用户会失去对简历的"理解"
