...
|------|--------------|----------|
| 场景、风格、版式、文案语言 | [OK] | — |
| quality-gate、Design Compliance 注释 | [OK] | — |
| 项目目录、assets 落盘、git 版本 | — | [OK] |
| `meta.json`、`send_file` 版本卡片 | — | [OK] |
...
|------|----------|------------|------------|
| **用户上传** | 先 HTML 后传图 / 先传图后 HTML | 从 `upload/` `cp` 到 `assets/`，引用与文件名一致；后传图时按现有 HTML 引用名命名 | 只 cp、未改代码 → [X]；改了 HTML 引用 → [OK] |
| **Agent 搜图** | 用户要配图、Agent 联网找图 | 下载到 `assets/{名}`（禁止仅写外链 URL）；再写/改 HTML 引用 | [OK] |
| **Agent 生图** | 用户要配图、Agent 生成图片 | 写入 `assets/{名}`（禁止只写 `assets/xxx` 不落盘）；再写 HTML | [OK] |
...
| 自然语言编辑 | [OK] | 版本号递增 |
| 评论功能编辑 | [OK] | 版本号递增 |
| 参数编辑器保存 | [OK] | 改文本、样式参数、替换图片均视为新版本 |
| 版本恢复 | [OK] | 见 §4 |
| 基于历史版本编辑 | [OK] | §4 合并，产生一个版本 |
| 用户仅上传资源、无代码变更 | [X] | 只写 `assets/`，不 commit |
| 用户上传资源且 Agent 更新了代码引用 | [OK] | 须 commit + `send_file` |
| 在同项目下新增入口文件（如新增 `light.html`） | [OK] | 新文件与现有文件一起提交为新版本 |
