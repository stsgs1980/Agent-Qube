...
- [X] 帮用户编数字（说"提升 30%"但用户根本没说过）—— 必须基于用户提供的信息，不知道就标 `[待补充：具体数字]`
- [X] 用形容词堆砌（"具有出色的沟通能力" "认真负责" "有上进心"）—— 删掉
- [X] 一段经历写超过 5 条 bullet —— 太密会被 HR 跳过
- [X] 写"自我评价"长篇 —— 国内现在不流行，1~2 行 summary 即可
- [X] 把所有公司都写一样的 bullet 数 —— 重要的多写，次要的少写
- [X] 把"熟悉 / 了解 / 会用"当能力描述 —— 改成"用 X 做了 Y"
