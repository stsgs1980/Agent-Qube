...
> [!] **不处理 study-buddy 的"即时练习"**——那条链路由 study-buddy 走外部 `exam_take`，不调本 skill。
...
[!] **本 skill 仅写入 USER.md 第 3 节"薄弱知识点"**（来源=`quiz-mastery`）；不写其他分区，也不写 `memory/`。其他持久化由 study-buddy 统一负责。
...
| 用户说 | 判定 | 行动 |
|---|---|---|
| "要 / 好 / 嗯 / 来一个 / 生成 / 网页 / 浏览器" | [OK] 要 | 调用 `quiz-html` |
| "不用 / 不要 / 算了 / 直接做 / 这里做" | [X] 不要 | 走原对话流程 |
| 没回应 / 不明确 | 默认 [X] 不要 | 直接走原流程，不强推 |
...
result = subprocess.run([
    ...
    str(qjson),
    "--title", page_title,        # 如 "[Books] 物理 · 电学练习"
    "--output", str(output),
    "--open",                     # 生成后自动用浏览器打开
...
- `category`：**一级分类，短词**（建议 2-6 字），用于网页顶部分类筛选 chip。
  - [OK] 推荐：`物理` / `数学` / `法律` / `历史` / `编程` / `通用`
  - [X] 避免：`通用类 / 1.中华人民共和国证券法（1998年12月29日…）` 这种长串、含日期/编号/斜杠的写法
