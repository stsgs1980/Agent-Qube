...
- [X] 给一份"通用 50 题"应付了事 → 必须按公司 / 岗位定制
- [X] 参考回答全是"我具有出色的沟通能力"这种空话 → 必须用具体故事
- [X] 不让用户自己练，全程"我替你答" → 错过模拟价值
- [X] 编造用户没有的经历做 STAR 故事 → 用户面试时会穿帮
- [X] HR 面问题答得太"训练痕迹重"（机械化） → 要给"3 个不同风格的版本"让用户选
